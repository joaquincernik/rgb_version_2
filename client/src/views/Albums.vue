...
<template>
    <div class="container">
        ...
        <div class="d-flex justify-content-center my-5">
            <nav v-if="!loadingAlbums && totalPages > 1" class="mt-3" aria-label="Paginación de álbumes">
                <ul class="pagination">
                    <!-- Previous -->
                    <li class="page-item" :class="{ disabled: page === 1 || loadingAlbums }">
                        <button class="page-link " type="button" @click="prev" :disabled="page === 1 || loadingAlbums">
                            Anterior
                        </button>
                    </li>

                    <!-- Si hay más de 5 páginas, mostrar primer página y puntos -->
                    <li v-if="pageNumbers[0] > 1" class="page-item">
                        <button class="page-link" type="button" @click="goTo(1)">1</button>
                    </li>
                    ...
                    <li v-for="p in pageNumbers" :key="p" class="page-item" :class="{ active: p === page }">
                        <button class="page-link" type="button" @click="goTo(p)">
                            {{ p }}
                            <span v-if="p === page" class="visually-hidden">(actual)</span>
                        </button>
                    </li>
                    ...
                    <li v-if="pageNumbers[pageNumbers.length - 1] < totalPages" class="page-item">
                        <button class="page-link" type="button" @click="goTo(totalPages)">{{ totalPages }}</button>
                    </li>

                    <!-- Next -->
                    <li class="page-item" :class="{ disabled: page === totalPages || loading }">
                        <button class="page-link" type="button" @click="next"
